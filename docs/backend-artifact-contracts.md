...
- `/api/v1/accounts/{ss58}/deregistrations`: fetch an account's neuron-deregistration footprint per subnet over a `?window=7d|30d|90d` — per-subnet NeuronDeregistered count with the first/last deregistration timestamps, account totals, an HHI concentration of where its exit activity is focused, and the dominant subnet — the account-level companion to `chain/deregistrations` (live from the `account_events` D1 tier).
