...
- `/metagraph/chain/performance/history.json`: schema for the per-day network-wide reward-flow and trust trend over a 7d/30d/90d window — computed live from the `neuron_daily` D1 rollup at `GET /api/v1/chain/performance/history` (no static file).
...
- `/api/v1/chain/performance/history`: fetch the per-day network-wide reward-flow and trust trend over a `?window=7d|30d|90d` (default 30d) — incentive/dividends concentration plus trust/consensus/validator_trust mean & median, one point per day (computed live from the `neuron_daily` D1 rollup; schema-stable empty series when cold).
