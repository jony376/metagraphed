...
- `/metagraph/chain/yield/history.json`: schema for the per-day network-wide emission-yield trend (aggregate network return plus validator/miner split and the per-neuron return distribution) over a 7d/30d/90d window across every subnet — computed live from the `neuron_daily` D1 rollup at `GET /api/v1/chain/yield/history` (no static file).
...
- `/api/v1/chain/yield/history`: fetch the per-day network-wide emission-yield trend over a `?window=7d|30d|90d` (default 30d) — the aggregate network return, validator vs miner split, and per-neuron return distribution, one point per day (computed live from the `neuron_daily` D1 rollup; schema-stable empty series when cold).
