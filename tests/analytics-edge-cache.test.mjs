...
describe("analytics edge cache", () => {
  ...
  test("chain yield history canonicalizes equivalent window query strings before caching", async () => {
    originalCaches = globalThis.caches;
    const cache = mockCaches();
    cache.install();
    const queries = [];
    const env = analyticsEnv(queries);
    const variants = [
      "https://api.metagraph.sh/api/v1/chain/yield/history?window=90d",
      "https://api.metagraph.sh/api/v1/chain/yield/history?window=90d&",
      "https://api.metagraph.sh/api/v1/chain/yield/history?window=90d&&",
    ];
